...
export const DocumentControllerApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        getDocumentLastVersion: async (metadataId: number, options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            // verify required parameter 'metadataId' is not null or undefined
            assertParamExists('getDocumentLastVersion', 'metadataId', metadataId)
            const localVarPath = `/API/documents/{metadataId}/versions/last`
                .replace(`{${"metadataId"}}`, encodeURIComponent(String(metadataId)));
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
...
export const DocumentControllerApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async getDocumentLastVersion(metadataId: number, options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<number>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.getDocumentLastVersion(metadataId, options);
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['DocumentControllerApi.getDocumentLastVersion']?.[localVarOperationServerIndex]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, localVarOperationServerBasePath || basePath);
        },
        /**
         * 
...
export const DocumentControllerApiFactory = function (configuration?: Configuration, basePath?: string, axios?: AxiosInstance) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        getDocumentLastVersion(metadataId: number, options?: any): AxiosPromise<number> {
            return localVarFp.getDocumentLastVersion(metadataId, options).then((request) => request(axios, basePath));
        },
        /**
         * 
         * @param {number} metadataId 
...
export class DocumentControllerApi extends BaseAPI {
     ...
     * @memberof DocumentControllerApi
     */
    public getDocumentLastVersion(metadataId: number, options?: RawAxiosRequestConfig) {
        return DocumentControllerApiFp(this.configuration).getDocumentLastVersion(metadataId, options).then((request) => request(this.axios, this.basePath));
    }
